...
stopuser = {}
token = "7337962051:AAEvGBjqbJZl96MVZZPX6mWZUJFKwQoeHt8"
...
- Programmer ~ @cheetax1 -  </b>''',reply_markup=keyboard)
		...
		keyboard.add(ba)
		keyboard.add(d)
		bot.reply_to(message, text=f'ϟ Choose The Gateway You Want to use from Bellow ',reply_markup=keyboard)
...
def enable_br2(message):
    ...
    if str(message.from_user.id) in admins:
        check_enabled_br2 = True
        bot.send_message(chat_id=message.chat.id, text='- Shopify 10$ 🌟 has been enabled. ✅')
    else:
        bot.send_message(chat_id=message.chat.id, text='- You are not the owner🤍')
...
def disable_br2(message):
    ...
    if str(message.from_user.id) in admins:
        ...
        bot.send_message(chat_id=message.chat.id, text='- Shopify 10$ 🌟 has been disabled. 🔒')
    else:
        bot.send_message(chat_id=message.chat.id, text='- You are not the owner🤍')
...
def handle_toggle(call):
    ...
    if call.data == 'toggle_br1':
        ...
        bot.answer_callback_query(call.id, f"Shopify 10$ 🌟 is now {status}.")
...
MAX_LINES = 1000
stopuser = {}  # لتعقب حالة كل مستخدم
check_enabled_br2 = True  # لتتبع ما إذا كان فحص Shopify 10 🌟 مفعلًا أم لا
...
def handle_admin_commands(message):
    ...
    if str(message.from_user.id) in admins:
        check_enabled_br2 = False
        bot.send_message(chat_id=message.chat.id, text='- Shopify 10$ 🌟 Check has been disabled. 🔒 No users can start the check until it is re-enabled.')
    else:
        bot.send_message(chat_id=message.chat.id, text='- You are not the owner🤍')
...
def handle_admin_commands(message):
    ...
    if str(message.from_user.id) in admins:
        check_enabled_br2 = True
        bot.send_message(chat_id=message.chat.id, text='- Shopify 10$ 🌟 Check has been re-enabled. ✅ Users can now start the check.')
    else:
        bot.send_message(chat_id=message.chat.id, text='- You are not the owner🤍')
...
MAX_LINES = 1000
stopuser = {}  # لتعقب حالة كل مستخدم
check_enabled_br3 = True  # لتتبع ما إذا كان فحص Braintree A on  uth 3 مفعلًا أم لا
...
- Programmer -> @cheetax1 </b>''', 
                    ...
                    msg = f'''<b>• Braintree Auth(Avs)-
┏━✨ CARD DETAILS ✨━
┣💳 Card:<code>{cc}</code>┣📋 Status: {last} 🟢
┣💬 Response: Auth Done ✅
┣🌐 Gate: {gate} 
－－－－－－－－－－－－－－－－{str(dato(cc[:6]))}
－－－－－－－－－－－－－－－－
...
┣💳 Card: <code>{cc}</code>
┣📋 Status: {last} 🟢
┣💬 Response: Auth Declined ⚰️
┣🌐 Gate: Braintree Auth(Non-Avs)
－－－－－－－－－－－－－－－－
 {str(dato(cc[:6]))}
－－－－－－－－－－－－－－－－
┏━✅ SUMMARY ✅━
